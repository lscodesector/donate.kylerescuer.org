 ...
 * ╚══════════════════════════════════════════════════════════════════════╝
 *
 * Esta é a campanha de `donate.kylerescuer.org` (WordPress + Elementor)
 * reconstruída aqui: o **conteúdo** é o de lá - história, abrigos, custos,
 * depoimentos, links -, e a **casca** é a do site da SOS Animal Help - o
 * desenho, os componentes, o fluxo de doação em modal e o checkout.
 *
 ...
 *  18 Checkout        dados e pagamento (z-70)
 *  19 ModalDocumento  o cartão CNPJ por cima de tudo (z-65)
 *  20 Pixel           eventos de conversão - montado em `app/layout.tsx`
 *
 * ── ⚠️ O bloco 05 saiu, e a numeração ficou ⚠️ ───────────────────────────
 * Era o "Pix direto": a chave da campanha solta na página, para quem preferia
 * pagar pelo app do banco. A campanha cobra em dólar e o único rail é o PayPal
 * (cartão ou conta) - uma chave Pix ali seria um caminho de pagamento que a
 * conciliação desta campanha não conhece.
 *
 * Os arquivos **não foram renumerados**: os nomes 06 a 20 são citados em
 * comentário por meia dúzia de arquivos, e trocar todos para fechar um buraco
 * na contagem custaria mais do que o buraco.
 */
