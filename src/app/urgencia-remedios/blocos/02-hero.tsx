 ...
 * mesmos +500 animais e 5 abrigos que a seção de transparência publica.
        ...
        <Reveal className="flex flex-col items-center gap-2">
          ...
          */}
          {/*
            ...
            contra os ~475-594px da coluna - é o que deixa o grito ("Eles
            precisam de veterinário agora!") fechar em uma linha do notebook
            para cima, com fonte de cartaz.

            ⚠️ O teto de 2,4rem é o limite medido: acima dele a fonte cresce
            mais rápido do que a largura disponível e a manchete ganha linha.
            Ao mexer nele, confira as duas pontas - 1366 e 1920.

            As duas frases são dois `block`, e não texto corrido com `balance`:
            a quebra cai sempre entre elas, que é onde a frase pede para
            respirar. Deixada ao critério do navegador, ela caía no meio da
            segunda oração e a manchete virava três linhas tortas.

            ── Por que a segunda linha é menor ───────────────────────────────
            Ela não é grito, é o dado que sustenta o grito ("+500 animais
            precisam de atendimento, exames e medicação urgente") - e tem quase
            o dobro de caracteres da primeira. No mesmo corpo da manchete, ela
            sozinha ocuparia duas ou três linhas de cartaz e empurraria o vídeo
            para fora da primeira dobra, que é o que a dobra inteira existe para
            evitar. O `0.62em` é medido **contra a própria manchete**, e não
            contra a raiz: os dois textos encolhem e crescem juntos, em qualquer
            tela, sem uma segunda escala para manter afinada.

            No celular cada uma delas ainda quebra sozinha, porque lá não existe
            largura - e é o que já acontecia antes.
          */}
          {/* Caixa alta na manchete inteira - as duas linhas, como a campanha
              a publica. É `uppercase` (CSS) e não texto gritado no `heroCopy`:
              o leitor de tela recebe a frase escrita normalmente, o corretor
              não vê uma sigla de sessenta letras, e voltar atrás é apagar uma
              palavra desta classe. O `tracking` abre um fio: caixa alta em
              peso 800 fecha demais as letras no tamanho de cartaz. */}
          <h1 className="w-full text-balance text-[clamp(1.279rem,0.883rem+1.256vw+0.465vh,2.232rem)] font-extrabold uppercase leading-[1.12] tracking-[0.01em] text-ink-900">
            <span className="block">{heroCopy.headline}</span>
            <span className="block text-[0.62em] leading-[1.22] text-action">
